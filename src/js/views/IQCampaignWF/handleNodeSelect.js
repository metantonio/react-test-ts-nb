...
export default (props) => {
    ...
    const urlQuery = "/campaignQuerySelectV3"
    ...
    const handleClick = async (event) => {
        ...
        if (response.ok) {
            ...
            setTotalrows(response.total_table)
            Swal.fire({
                position: 'top-end',
                icon: 'success',
                title: 'Query made',
    ...
    return (<>
        <div
            style={{
                ...
                overflowY: showTable ? 'auto' : 'initial'
            }}
        >
            ...
            >
                ...
                {!tableNode || tableNode.length == 0 ? <> <button className="nodrag btn btn-primary text-dark" onClick={handleClick} type='button'>
                    ...
                    <><button className="nodrag btn btn-primary text-dark" onClick={handleClick} type='button'>
                        ...
                        {/* <Excel idDiv={`node-${props.id}`} />  */}<button className="nodrag btn btn-primary text-dark" style={{ margin: '5px' }} onClick={(e) => { setShowQuerySQL(!showQuerySQL) }} type='button'>{!showQuerySQL ? "Show SQL Query" : "Hide SQL Query"}</button> Rows: {tableNode.length} - {(100 * tableNode.length / totalrows).toFixed(2)}%</>}
            ...
            <div
                style={{
                    ...
                    flexDirection: 'row',
                    alignItems: 'center',
                    justifyContent: showTable ? 'space-between' : 'space-evenly'
                }}
            >
