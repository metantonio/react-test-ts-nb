        ...
        >
            ...
            >
                ...
                {!tableNode || tableNode.length == 0 ? <button className="nodrag btn btn-primary text-dark" onClick={handleClick} type='button'>
                    Make Query
                </button> : <><button className="nodrag btn btn-primary text-dark" onClick={handleClick} type='button'>
                    Repeat Query
                </button> {/* <Excel idDiv={`node-${props.id}`} />  */}<button className="nodrag btn btn-primary text-dark" style={{ margin: '5px' }} onClick={(e) => { setShowQuerySQL(!showQuerySQL) }} type='button'>{!showQuerySQL ? "Show SQL Query" : "Hide SQL Query"}</button> Rows: {tableNode.length}</>}
