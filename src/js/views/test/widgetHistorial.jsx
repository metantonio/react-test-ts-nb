...
const WidgetHistorial = () => {
    ...
    return (
        <div className={styles.window1}>
            <button className={`${styles.ai_credits_text}`} type="button" data-bs-toggle="offcanvas" data-bs-target="#widgethistorial" aria-controls="widgethistorial" style={{border:"none", background:"none"}}>History</button>
            <div className="offcanvas offcanvas-start" data-bs-scroll="true" tabindex="-1" id="widgethistorial" aria-labelledby="widgethistorialLabel" style={{ width: "670px", overflowY: "scroll", backgroundColor: "#f9f9f9" }}>
                <div className="offcanvas-header">
                    <h2 className="offcanvas-title" id="widgethistorialLabel">Latest {number} prompts <button type='button' className={`${styles.nav_button} ${styles.import_button}`} onClick={(e) => { setReload(!reload) }}>🔄️</button></h2>
                ...
                <div className="offcanvas-body">
                    <>
                        <div className='row d-flex justify-content-center align-items-center mb-2'>
                            ...
                            <div className='col'><button type='button' className={`${styles.nav_button} ${styles.import_button}`} onClick={(e) => { setReload(!reload) }}>Apply</button></div>
                        </div>
                                ...
                                {listFiles.length > 0 ? listFiles.filter(item => {
                                    ...
                                    return (
                                        <div href="#" className="list-group-item list-group-item-action active" aria-current="true" key={index} style={{ color: "black", lineHeight: "normal", backgroundColor: "white", borderColor: "#ececec" }}>
                                            <p className="text-wrap" style={{ display: "flex", fontSize: "0.9em", justifyContent: "start", alignItems: "center", inlineSize:"max-content" }}>
                                                Prompt: {item["prompt_id"]["prompt"]}
                                            </p>
                                            <p style={{ lineHeight: "normal", display:"flex", inlineSize:"max-content" }}><small>{item["prompt_id"]["registered"]}</small></p>
                                        </div>
                                    )
                                }) : <></>}
